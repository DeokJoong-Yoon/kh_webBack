...
public class MemberLogInServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 로그인 처리를 위한 메서드 호출. 아이디와 비밀번호 일치 시 MemberDTO의 인스턴스 값 반환.
		// 일치하지 않을 시 null 반환
		...
		if(memberDTO != null) { // 로그인 성공시
			...
			message.append("<div><a href='/servletExample/loginInfo'>로그인 정보 확인하기");
		...
		// 정상적으로 로그인 되었을 대. 세션 객체의 속성으로 접근
		...
		if (mDTO != null) {
			out.print(mDTO.getMemberName() + "님 반갑스니다.");
		}
