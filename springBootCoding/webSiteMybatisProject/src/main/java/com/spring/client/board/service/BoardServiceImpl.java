...
//	private ReplyDao replyDao;
	...
	/*
	 * @Override public int boardInsert(BoardVO bvo) throws Exception { int result =
	 * 0; if (bvo.getFile().getSize() > 0) { String fileName =
	 * FileUploadUtil.fileUpload(bvo.getFile(), "board"); //
	 * board_1658205347_cat.jpg bvo.setBoardFile(fileName); } result =
	 * boardDao.boardInsert(bvo); return result; }
	 */
	...
	/*
	 * @Override public int boardUpdate(BoardVO bvo) throws Exception { int result =
	 * 0; if (!bvo.getFile().isEmpty()) { // 새롭게 업로드할 파일이 존재하면 if
	 * (!bvo.getBoardFile().isEmpty()) { // 기존 파일이 존재하면
	 * FileUploadUtil.fileDelete(bvo.getBoardFile()); }
	 * 
	 * String fileName = FileUploadUtil.fileUpload(bvo.getFile(), "board");
	 * bvo.setBoardFile(fileName); } result = boardDao.boardUpdate(bvo); return
	 * result; }
	 */
	/*
	...
	/*
	 * @Override public int boardDelete(BoardVO bvo) { int result = 0; if
	 * (!bvo.getBoardFile().isEmpty()) { // boardFile 필드의 값이 null 이거나 "" 아니면 (이미지
	 * 파일이 존재하면) FileUploadUtil.fileDelete(bvo.getBoardFile()); } result =
	 * boardDao.boardDelete(bvo); return result; }
	 */
	 ...
	 */
	/*
	 * @Override public int replyCount(int boardNumber) { int result = 0; result =
	 * replyDao.replyCount(boardNumber); return result; }
	 */
